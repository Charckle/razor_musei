...
class Pylavor:
    ...
    # Pylavor
    @staticmethod
    def datetime_to_string_time(date_n_time, date_=True, hours_=True, minutes=True, seconds=True) -> str:
        if isinstance(date_n_time, str):
            date_n_time = datetime.fromisoformat(date_n_time)            
        if not isinstance(date_n_time, str):
            ...
            date_n_time = date_n_time.strftime(format_string)
        else:
    ...
    def clean_rich_text(input_text):
        ...
        allowed_attributes = {'a': ['href', 'title'], 'img': ['style', 'src']}
        
        allowed_protocols = ['https', 'data']  # Allow data:image/...

        css_sanitizer = CSSSanitizer(allowed_css_properties=['width', 'height'])
    
        # Use bleach to clean the input text
        cleaned_text = bleach.clean(input_text, tags=allowed_tags, attributes=allowed_attributes, protocols=allowed_protocols, css_sanitizer=css_sanitizer)
